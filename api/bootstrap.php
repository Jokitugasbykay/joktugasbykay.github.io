...
function db(): PDO { 
    ...
    // Keep the runtime schema in sync with database/schema.sql. Existing databases
    // are upgraded below so a stale cPanel SQLite file does not break checkout.
    $schema = "
    ...
    if (!in_array('service_id', $names, true)) $pdo->exec("ALTER TABLE orders ADD COLUMN service_id TEXT NOT NULL DEFAULT 'custom'");
    if (!in_array('paid_at', $names, true)) $pdo->exec('ALTER TABLE orders ADD COLUMN paid_at DATETIME');
    $pdo->exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_mayar_transaction ON orders(mayar_transaction_id) WHERE mayar_transaction_id IS NOT NULL');
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(payment_status)');
    $pdo->exec("CREATE TABLE IF NOT EXISTS webhook_deliveries (transaction_id TEXT PRIMARY KEY, event_id TEXT, status TEXT NOT NULL DEFAULT 'completed', attempt_count INTEGER NOT NULL DEFAULT 1, last_error TEXT, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, completed_at DATETIME)");
    return $pdo; 
}
